...
void initialize_labels(string[]);
void initialize_delays(int[]);
int getaddroflabel(string, string[], int[]);
int reg_stoi(string);
int main(){
     ifstream file,file1;
     file.open("input.txt");
     ...
     for(int i=0; i< 20; i++){
          cout << i <<" = " << labels[i] << " = " << labeladdr[i] << endl;
     }
     //------------------------------
     ...
     cout << "------------------------------------------------------------------------------" << endl;
     cout<<"INITIAL STATUS OF REGISTER"<< endl;
     printreg(reg);
     pc=0;
     while(mem[pc].compare("00")!=0){
          ...
          if(inst.compare("add")==0){
               ...
               getline(parse, label, ' ');
               int intlabel=getaddroflabel(label,labels,labeladdr);
               // cout << " = " <<intlabel << endl;
               pc=intlabel-1;
               totalcycle+=delays[6];
          }
     ...
     cout << "Average instruction per cycle: " << ((float)instcount/(float)totalcycle) << endl;
     return 0;
}
...
int getaddroflabel(string label, string labels[], int labeladdr[]){
     // cout <<"$$$$$$"<<endl;
     for(int i=0; i<20; i++){
          string templabel;
          stringstream parse1(labels[i]);
          getline(parse1,templabel, ' ');
          // cout << "=" << templabel << "," << label << endl;
          if(label.compare(templabel)==0){
               return labeladdr[i];
          }
     }
     // cout <<"$$$$$$"<<endl;
...
void initialize_labels(string labels[]){
     for(int i=0; i<20; i++){
          labels[i]="";
     }
}
