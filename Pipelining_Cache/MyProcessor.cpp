...
int reg[32], pc, clk, done, inst_count, number_of_instructions_executed, miss_count, hit_count;
string mem[4096];
...
int check_hit(double x) {
	random_device rd;
	mt19937 e2(rd());  // Mersenne twister MT19937
	uniform_real_distribution<> dist(0, 1);
	double random_double = dist(e2);
	if (random_double <= x) return HIT;
	else return MISS;
}
...
public:
	...
	void fetch() {
		...
		if (miss_wait) {
			buffer_if << "IF Waiting (Cache miss)" << endl;
			return; 
		}
	...
	void decode() {
		...
		if (miss_wait) {
			buffer_id << "ID Waiting (Cache miss)" << endl;
			return; 
		}
	...
	void execute() {
		...
		if (miss_wait) {
			buffer_exe << "EXE Waiting (Cache miss)" << endl;
			return; 
		}
	...
	void ma() {
		...
		if (miss_wait) {
			buffer_mem << "MEM Waiting (Cache miss) processing instruction " << ex_mem_latch->instruction_number << "-> " << mem[ex_mem_latch->instruction_number] << endl;
		...
		if (ex_mem_latch->sw) {
			if (!miss_waited) {
				if (!check_hit(hit_probability)) {
					buffer_mem << "\tCache MISS" << endl;
					miss_wait = access_cycles - 1;
					temp_mem_wb_latch = nullptr;
					++miss_count;
					/* if there is a cache miss then the registers that were updated in
					the preious and previous to previous cycles should be set as updated
					the clk - 1 + N - 1 and clk - 2 + N - 1 th cycles respectively */
					for (int i = 0; i < 32; ++i) {
						if (last_written[i] == clk - 1) last_written[i] += access_cycles - 1;
						if (last_loaded[i] == clk - 1) last_loaded[i] += access_cycles - 1;
						if (last_written[i] == clk - 2) last_written[i] += access_cycles - 1;
						if (last_loaded[i] == clk - 2) last_loaded[i] += access_cycles - 1;
					}
					return;
				}
				else {
					buffer_mem << "\tCache HIT" << endl;
					++hit_count;
				}
			}
	...
	void wb() {
		...
		if (miss_wait) {
			buffer_mem << "WB Idle (Cache miss)" << endl;
			temp_mem_wb_latch = nullptr;
			return; 
		}
	...
	PipeLine() {
		...
		pc = 0, clk = 0, done = 0, inst_count = 0, number_of_instructions_executed = 0, miss_count = 0, hit_count = 0;
		...
		while (1) {
			...
			fetch();
			buffers(1);
			if (!miss_wait) id_ex_latch = temp_id_ex_latch;
			if (!miss_wait) ex_mem_latch = temp_ex_mem_latch;
			if (!miss_wait) mem_wb_latch = temp_mem_wb_latch;
			if (!miss_wait) post_wb_latch = temp_post_wb_latch;
			if (!miss_wait) {
				if (if_id_latch != nullptr && if_id_latch->stall) {
					--if_id_latch->stall;
			...
			if (done) {
				...
				file2 << "Hit Count: " << hit_count << endl;
				file2 << "Miss Count: " << miss_count << endl;
				break;
			}
		}
	}
